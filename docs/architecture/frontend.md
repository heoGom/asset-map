...
- 월별 배당 차트는 지급 데이터가 없는 달도 1~12월 축을 유지해 현황 영역이 사라지지 않게 표시합니다.
- `SecurityCombobox`는 ticker 숫자 오름차순 정렬, 검색 자동완성, 방향키/Enter 선택을 지원하며 거래 입력 등 폼 내부에서 Enter가 의도치 않은 submit으로 흐르지 않게 처리합니다.
- Header의 `관리` 메뉴는 1차 메뉴에서 낮은 우선순위로 유지하고, 마우스 hover/focus 시 하단 submenu로 종목 관리를 노출합니다.
