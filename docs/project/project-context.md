...
- **핵심 도메인**:
  ...
  - 거래(`TradeTransaction`): 매수/매도/초기보유 원천 거래내역
  - 보유(`Holding`): 거래내역 반영 결과로 유지하는 현재 보유 상태 캐시
- **이력 및 분석**:
  - 스냅샷(`HoldingSnapshot`): 주기적인 자산 상태 저장 및 타임라인 분석
  - 배당(`Dividend`): 배당 이벤트(공공데이터 연동), 기준일 보유수량 기반 예상 지급 내역 자동 생성, 실제 지급 내역 관리
  - 시세(`MarketPrice`): KRX API 연동을 통한 일별 시세 수집 및 보유 종목 현재가 갱신
- **외부 데이터 동기화 (Admin Sync)**:
  - KRX 종목 마스터: 유가증권/코스닥 전체 종목 upsert
  - KRX 일별 시세: 내 거래 종목 대상 backfill 및 정기 수집
  - 국내 주식 배당: 금융위원회 API 기반 배당 이벤트 및 사용자별 예상 지급 내역 자동 생성
- **대시보드 및 UI**:
  - 자산 요약, 비중 통계, 월별/연도별 배당 성장률 분석 API
  - Next.js 16.2 (App Router) 기반 홈, 자산, 배당, 계좌, 종목 관리 화면 구현 완료
  - 검색 가능한 종목 선택 자동완성(`SecurityCombobox`) UI 적용
