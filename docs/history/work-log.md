...
### 대시보드 UI와 배당 집계 표시 보정

- Header의 `관리` 메뉴를 hover/focus 시 하단 submenu로 노출되게 조정하고, 메뉴와 submenu 사이 hover 영역이 끊기지 않게 보정해 종목 관리 접근을 유지했다.
- 자산 타임라인은 저장된 스냅샷이 없더라도 현재 보유 평가액이 있으면 오늘 기준 1점을 반환해 차트 영역이 비어 보이지 않게 했다.
- 월별 배당 차트는 지급 데이터가 없는 달도 1~12월 축을 유지해 현황 영역이 항상 보이도록 했다.
- 배당 대시보드는 `PAID` 상태와 지급일이 지난 생성 배당금을 수령액으로 집계하고, 과거 분기 빈도를 이용해 일부 분기만 들어온 종목의 연간 예상 배당금을 연간화한다.
- 배당 집계와 스냅샷 fallback 동작을 단위 테스트로 고정했다.
